...
def calSS(trace_id):
    ...
    session = Session()
    query = f""" INSERT INTO suspiciousness_scores (SELECT g.name, {trace_id}, g.score 
                FROM( select a.name, c.bytes_total, c.bytes_min, c.bytes_max, b.flows_total, 
                ...
                and ip_dst <> ' ' and trace_id = {trace_id} group by d.name) a,
                ...
                FROM packet_logs l, devices d WHERE d.ipv4 = l.ip_src and ip_src <> ' ' 
                and ip_dst <> ' ' and trace_id = " + trace_id + " group by d.name) b, 
                ...
                FROM packet_logs l, devices d WHERE d.ipv4 = l.ip_src and ip_src <> ' ' 
                and ip_dst <> ' ' and (trace_id = {trace_id})  group by d.name) c 
