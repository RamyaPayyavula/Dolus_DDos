...
from python.models import PacketLogs


def calSSByTime(trace_id):
    session = Session()
    # findMin = session.query(func.min(Product.price), func.max(Product.price)).filter(Product.title=="Jones").one()
    findMin = session.query(func.min(PacketLogs.frame_time), func.max(PacketLogs.frame_time)).filter(PacketLogs.trace_id==traceid).one()
    minFrame = findMin[0]
    maxFrame = findMin[1]
    try:
        while minFrame < maxFrame:
            query = f"""INSERT INTO suspiciousness_scores_by_time VALUES({minFrame},
            		...
            		AND trace_id = {traceid} AND frame_time >= {minFrame}
            		AND frame_time < {minFrame} + 10 GROUP BY d.name) a, 
            		...
            		AND trace_id = {traceid} AND frame_time >= {minFrame} AND frame_time <{minFrame}+ 10 GROUP BY d.name) b, 
            		...
            		FROM packet_logs l, devices d WHERE d.ipv4 = l.ip_src AND ip_src <> ' ' AND ip_dst <> ' ' 
            		AND trace_id = {traceid} AND frame_time >= {minFrame} AND frame_time < {minFrame} + 10 
