...
def main():
    ...
    while True:
        ...
        existing_policies = pf.getExistingPolicy()

        current_policy,ids = pf.getCurrentPolicy()
        ...
        if current_rule == temp:
            ...
            if len(temp) == 0:
                ...
                for id in ids:
                    ...
                    # db.commit()
                    session.query(Policies).filter(Policies.loaded ==0).delete()
                    session.commit()
                ...
                try:
                    ...
                    for id in ids:
                        session.query(Policies).filter(Policies.policyID==id).update({"loaded":1})
                        session.commit()
                ...
                #db.close()
                    ...
                    for id in ids:
                        ...
                        # #policy = cursor.fetchall()
                        # db.commit()
                        query = session.query(Policies).filter(Policies.policyID == id).delete()
                        session.commit()
                        print(id)
